...
#pragma region RootParameter
	...
	rootParamaters[3].Descriptor.ShaderRegister = 1;//レジスタ番号１
...
#pragma endregion
	...
	ID3D12Resource* directionalLightResoruce = CreateBufferResource(device, sizeof(DirectionalLight));
	DirectionalLight* directionalLightData = nullptr;
	directionalLightResoruce->Map(0, nullptr, reinterpret_cast<void**>(&directionalLightData));
...
#pragma region スフィアの表示
	...
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex) {
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
			...
			vertexData[index + 3].normal.x = vertexData[index + 2].position.x;
			vertexData[index + 3].normal.y = vertexData[index + 2].position.y;
			vertexData[index + 3].normal.z = vertexData[index + 2].position.z;
			...
			vertexData[index + 4].normal.x = vertexData[index + 1].position.x;
			vertexData[index + 4].normal.y = vertexData[index + 1].position.y;
			vertexData[index + 4].normal.z = vertexData[index + 1].position.z;
...
#pragma endregion
	...
	//単位行列を書き込んでおく
	transformationMatrixDataSprite->WVP = MekeIdentity4x4();
	transformationMatrixDataSprite->World = MekeIdentity4x4();
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			Matrix4x4 viewMatrixSprite = MekeIdentity4x4();
